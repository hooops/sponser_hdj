...
from new_event.models import NewEventCat, NewDistrict
    ...
    if request.method == 'GET':
        ...
        if record_line is None:
            ...
            for record in result:
                ...
                # 如果存在城市，则搜索
                if place:
                    query = [{'name': city.district_name, 'id': city.id } for city in NewDistrict.objects.filter(district_name__icontains=place)]
                ...
                cat = record[3]
                if cat:
                    q_cat = NewEventCat.objects.filter(name__icontains=cat)
            ...
            # 返回
            response = []
            for i, record in enumerate(new_result):
                if len(record) < 4:
                    continue
                city = record[2]
                cat = record[3]
            ...
            return render_to_response( \
                'friendlylink.html', \
                {'num': num, 'data': response,'result':new_result,'data_post':data},\
                context_instance=RequestContext(request)
                )
        else:
            # 数据确认
